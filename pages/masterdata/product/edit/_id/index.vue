                ...
                <div class="card border-top-primary shadow h-100 py-2">
                    <form @submit.prevent="simpan">
                        <div class="card-body">
                            ...
                            <div class="form-group row mb-4" v-if="data.old_image != ''">
                                <label for="description" class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Gambar Lama:</label>
                                <div class="col-sm-12 col-md-7">
                                    <div class="card" style="width: 20rem">
                                        <img :src="src+data.old_image" alt="Card image cap">
                                        <div class="card-body">
                                            <h5 class="card-titel">{{ data.old_image }}</h5>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="form-group row mb-4">
                                <label for="description" class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Gambar Baru:</label>
                                <div class="col-sm-12 col-md-7">
                                    <div class="card" style="width: 20rem">
        ...
        methods: {
            simpan() {
                ...
                this.$swal({
                    ...
                    this.$axios.$post('/update-product/'+this.id, formData, headers)
                    .then((response) => {
            ...
            getProduct() {
                this.$axios.$get('/product/'+this.id, {
                    ...
                    this.data.selling_price = data.selling_price
                    this.data.old_image = data.image
                    var idcat = []
                    data.category.forEach(function(item, index) {
                        idcat.push(item.category_id)
                    })
                    this.data.category = idcat
                }).catch((error) => {
                    console.log(error)
                    this.$swal({
                        text: 'Terjadi kesalahan, harap hubungi admin!',
